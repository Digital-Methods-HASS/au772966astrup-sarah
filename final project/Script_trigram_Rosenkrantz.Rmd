...
list.files(path = "breve_rosenkrantz", pattern = "pdf")
```
```{r, include=FALSE}
#Following the guide from software carpentry 
list.files(path = "breve_rosenkrantz", pattern = "brev_Rosenkrantz-", full.names = TRUE)
```
```{r, indsætter dokument}
#Following the guide from software carpentry 
filenames <- list.files(path = "breve_rosenkrantz",  
                        pattern = "brev_Rosenkrantz-.*\\.pdf$",
                        full.names = TRUE)
for (f in filenames) {
  print(f)
  analyze(f)
...
filenames <- list.files(
  path = "breve_rosenkrantz",  
  pattern = "brev_Rosenkrantz-.*\\.pdf${2}",
  full.names = TRUE
)
